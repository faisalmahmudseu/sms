...
$sql = "SELECT pid, project.eid, pname, duedate, subdate, mark, points, firstName, lastName, base, bonus, total FROM `project` , `rank` ,`student`, `mark`  WHERE project.eid = $id AND pid = $pid";
...
if(isset($_POST['update']))
{
 ...
 $result = mysqli_query($conn, "UPDATE `mark` SET `bonus`='$upBonus' ,`total`='$upmark' WHERE id=$eid");
...
<?php
  $id = (isset($_GET['id']) ? $_GET['id'] : '');
  $pid = (isset($_GET['pid']) ? $_GET['pid'] : '');
  $sql1 = "SELECT pid, project.eid, project.pname, project.duedate, project.subdate, project.mark, rank.points, student.firstName, student.lastName, mark.base, mark.bonus, mark.total FROM `project` , `rank` ,`student`, `mark`  WHERE project.eid = $id AND project.pid = $pid AND project.eid = rank.eid AND mark.id = project.eid AND student.id = project.eid AND student.id = rank.eid";
...
<html>
<head>
  <title>Project Marks | Student Management System</title>
  <!-- Icons font CSS-->
  ...
  <div class="page-wrapper bg-blue p-t-100 p-b-100 font-robo">
        <div class="wrapper wrapper--w680">
            <div class="card card-1">
                ...
                <div class="card-body">
                    <h2 class="title">Project Marks</h2>
                    <form id = "registration" action="mark.php" method="POST">
                        ...
                        <div class="p-t-20">
                            <button class="btn btn--radius btn--green" type="submit" name="update">Assign Marks</button>
                        </div>
